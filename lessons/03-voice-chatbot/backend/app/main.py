...
class RealtimeSecretRequest(BaseModel):
    ...
    ttl_seconds: int = Field(600, ge=10, le=7200)
...
@app.post("/api/realtime/client_secret", response_model=RealtimeSecretResponse)
def create_realtime_client_secret(req: RealtimeSecretRequest):
    """Mint a short-lived client secret for the browser to connect to OpenAI Realtime (WebRTC).

    We attach a baseline session configuration (model, voice, VAD interrupt, transcription).
    The browser will then connect directly to OpenAI using WebRTC with this ephemeral key.
    """
    if not OPENAI_API_KEY:
        raise HTTPException(status_code=500, detail="Missing OPENAI_API_KEY in backend .env")

    if req.doc_id not in DOCS:
        raise HTTPException(status_code=404, detail="Unknown doc_id. Upload a PDF first.")

    # Baseline instructions: Arabic by default, scope limited to PDF
    instructions = "\n".join([
        "You are a voice assistant that answers questions ONLY using the uploaded PDF document.",
        "If the user asks something not covered by the PDF, say you cannot answer because it is outside the document.",
        "Default language: Arabic. If the user speaks another language, reply in that language.",
        "Be concise and clear. When you use information from sources, cite them like [S1], [S2].",
    ])
            ...
            "audio": {
                "input": {
                    ...
                    "transcription": {
                        ...
                        # Leave language unset to support multilingual speech
                    },
                    "turn_detection": {
                        "type": "server_vad",
                        # We will create responses manually after retrieval
                        "create_response": False,
                        # Let user interrupt the model by speaking
    ...
    try:
        ...
        # cs = session.get("client_secret") or {}
        # value = cs.get("value")
        # expires_at = cs.get("expires_at") or data.get("expires_at")
        # if not value:
        #     raise HTTPException(status_code=500, detail=f"Unexpected OpenAI response: {data}")
        ...
        # A) { "value": "...", "expires_at": 123, "session": {...} }
        # B) { "client_secret": { "value": "...", "expires_at": 123 }, "type": "...", ... }
        # C) { "session": { "client_secret": { "value": "...", "expires_at": 123 }, ... }, "expires_at": ... }
        value = (
            data.get("value")
            or (data.get("client_secret") or {}).get("value")
            or ((data.get("session") or {}).get("client_secret") or {}).get("value")
        )

        expires_at = (
            data.get("expires_at")
            or (data.get("client_secret") or {}).get("expires_at")
            or ((data.get("session") or {}).get("client_secret") or {}).get("expires_at")
        )
...
def rag_retrieve(req: RetrieveRequest):
    if req.doc_id not in DOCS:
        raise HTTPException(status_code=404, detail="Unknown doc_id. Upload a PDF first.")
    doc = DOCS[req.doc_id]
    results = _retrieve(doc, req.query, req.k)
    sources = []
